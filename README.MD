...
Repack
==========
This lib was repack and use to replace Dapper.Contrib to work for [MsSQL, MySql] SQL Syntax and was taken from https://github.com/tofilagman/Dapper.SqlBuilder
...
<configuration>
    <packageSources>        
        <add key="githubDavidViolet" value="https://nuget.pkg.github.com/DavidViolet18/index.json" />
    </packageSources>
...
## Select with custom mysql function in conditions
```csharp
...
// => SELECT Users.* FROM Users WHERE Users.Id = @Param1
var copyQuery = query.Clone().SelectCount(x => x.Id);
// => SELECT COUNT(Users.Id) FROM Users WHERE Users.Id = @Param1 
```
